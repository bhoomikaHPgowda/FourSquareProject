...
import Foundation
class NetworkManagerForSearch {
    
    func searchPlacedetail(placeName: String, completionHanlderL: @escaping(([PlaceDetail]) -> ())) {
            print("function called")
           
            
        guard let url = URLs.fetchSearchedCityDetail(placeName: placeName)  else {
                print("wrong url")
                return
            }
           print("url === \(url)")
